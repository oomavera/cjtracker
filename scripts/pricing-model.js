...
function standardize(cols, rows) {
    const means = {};
    const stds = {};
    for (const c of cols) {
        const v = rows.map(r => Number(r[c]));
...
function designMatrix(rows, numeric, binary, clutterLevels, means, stds) {
    const X = [];
    for (const r of rows) {
        const x = [1]; // intercept
        for (const c of numeric)
            x.push((Number(r[c]) - means[c]) / stds[c]);
        for (const b of binary)
            x.push(Number(r[b]));
        // one-hot for clutter in the given order
        const cl = (r.clutter || "").toString();
        for (const lev of clutterLevels)
            x.push(cl === lev ? 1 : 0);
        X.push(x);
    }
    return new Matrix(X);
}
function ridgeFit(X, y, alpha, interceptIndex = 0) {
...
function kfoldIndices(n, k, seed = 42) {
...
// ----- Training -----
function trainRidge(data, alphaGrid = Array.from({ length: 50 }, (_, i) => Math.pow(10, -3 + i * (6 / 49))), kFold = Math.min(5, Math.max(2, Math.floor(data.length)))) {
...
function bootstrapPI(train, query, B = 2000, seed = 42) {
    const rng = (function* () { let r = seed >>> 0; while (true) {
        r = (r * 1664525 + 1013904223) >>> 0;
        yield r / 2 ** 32;
    } })();
...
function rateCheckHours(q, rates) {
    if (!rates || !isFinite(rates.baseRateSqftPerHour) || rates.baseRateSqftPerHour <= 0)
        return NaN;
...
function buildMatrix(model, data, opts) {
    ...
    for (let sqft = sqftStart; sqft <= sqftEnd; sqft += sqftStep) {
        for (const br of bedrooms)
            for (const ba of bathrooms)
                for (const d of deepVals)
                    for (const c of clutters)
                        for (const f of appl)
                            for (const o of appl) {
                                const q = { sqft, bedrooms: br, bathrooms: ba, deep_clean: d, clutter: c, fridge: f, oven: o };
...
const query = { sqft: 2390, bedrooms: 4, bathrooms: 2, deep_clean: 1, clutter: "medium", fridge: 0, oven: 0 };
